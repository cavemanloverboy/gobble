...
pub struct InvokeContext<'a, 'ix_data> {
    ...
    /// Byte offset ranges within instruction sysvar data that are forbidden to read.
    /// Used by the gobble detector to track forbidden memory accesses at the VM level.
    pub forbidden_sysvar_data_ranges: Vec<std::ops::Range<usize>>,
    /// Accumulated forbidden memory accesses detected during VM execution.
    /// Each entry is (vm_address, access_size).
    pub forbidden_memory_accesses: Vec<(u64, usize)>,
}
...
impl<'a, 'ix_data> InvokeContext<'a, 'ix_data> {
    ...
    pub fn new(
        ...
        Self {
            ...
            #[cfg(feature = "sbpf-debugger")]
            debug_port: None,
            forbidden_sysvar_data_ranges: Vec::new(),
            forbidden_memory_accesses: Vec::new(),
        }
    }

    /// Set the forbidden sysvar data byte offset ranges for memory tracking.
    pub fn set_forbidden_sysvar_data_ranges(&mut self, ranges: Vec<std::ops::Range<usize>>) {
        self.forbidden_sysvar_data_ranges = ranges;
    }

    /// Get the accumulated forbidden memory accesses detected during VM execution.
    pub fn get_forbidden_memory_accesses(&self) -> &[(u64, usize)] {
        &self.forbidden_memory_accesses
    }
