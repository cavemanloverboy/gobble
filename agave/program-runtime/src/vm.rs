...
pub fn execute<'a, 'b: 'a>(
    ...
    // Compute forbidden VM address ranges from sysvar data offsets.
    // Find the instructions sysvar account among the instruction accounts and
    // translate byte offsets into actual VM addresses.
    let forbidden_vm_ranges: Vec<std::ops::Range<u64>> =
        if !invoke_context.forbidden_sysvar_data_ranges.is_empty() {
            let instructions_sysvar_id = solana_sdk_ids::sysvar::instructions::id();
            let mut sysvar_vm_data_addr = None;

            let num_accounts = instruction_context.get_number_of_instruction_accounts();
            for i in 0..num_accounts {
                if let Ok(tx_idx) =
                    instruction_context.get_index_of_instruction_account_in_transaction(i)
                {
                    if let Ok(key) = invoke_context
                        .transaction_context
                        .get_key_of_account_at_index(tx_idx)
                    {
                        if *key == instructions_sysvar_id {
                            if let Some(meta) = accounts_metadata.get(i as usize) {
                                sysvar_vm_data_addr = Some(meta.vm_data_addr);
                            }
                            break;
                        }
                    }
            ...
            if let Some(base_addr) = sysvar_vm_data_addr {
                invoke_context
                    .forbidden_sysvar_data_ranges
                    .iter()
                    .map(|r| {
                        (base_addr + r.start as u64)..(base_addr + r.end as u64)
                    })
                    .collect()
            } else {
                Vec::new()
    ...
    let execution_result = {
        ...
        // Add forbidden memory watched ranges to the VM's access tracker
        if !forbidden_vm_ranges.is_empty() {
            vm.add_watched_memory_ranges(&forbidden_vm_ranges);
        }
        ...
        // Collect forbidden memory accesses from the VM tracker before dropping
        if vm.has_forbidden_memory_accesses() {
            let accesses = vm.get_forbidden_memory_accesses();
            vm.context_object_pointer
                .forbidden_memory_accesses
                .extend(accesses);
        }
