...
impl<'a, C: ContextObject> EbpfVm<'a, C> {
    ...
    /// Add a watched memory range for forbidden access detection
    pub fn add_watched_memory_range(&mut self, range: std::ops::Range<u64>) {
        if let Some(tracker) = self.memory_mapping.get_access_tracker() {
            tracker.add_watched_range(range);
        }
    }

    /// Add multiple watched memory ranges for forbidden access detection
    pub fn add_watched_memory_ranges(&mut self, ranges: &[std::ops::Range<u64>]) {
        if let Some(tracker) = self.memory_mapping.get_access_tracker() {
            tracker.add_watched_ranges(ranges);
        }
    }

    /// Check if any forbidden memory accesses were detected
    pub fn has_forbidden_memory_accesses(&self) -> bool {
        self.memory_mapping
            .get_access_tracker()
            .map(|t| t.has_forbidden_accesses())
            .unwrap_or(false)
    }

    /// Get list of forbidden memory accesses that were detected
    pub fn get_forbidden_memory_accesses(&self) -> Vec<(u64, usize)> {
        self.memory_mapping
            .get_access_tracker()
            .map(|t| t.get_forbidden_accesses())
            .unwrap_or_default()
    }

    /// Get the watched memory ranges
    pub fn get_watched_memory_ranges(&self) -> Vec<std::ops::Range<u64>> {
        self.memory_mapping
            .get_access_tracker()
            .map(|t| t.get_watched_ranges())
            .unwrap_or_default()
    }

    /// Reset the access tracker, clearing all recorded accesses but keeping watched ranges
    pub fn reset_access_tracker(&mut self) {
        if let Some(tracker) = self.memory_mapping.get_access_tracker() {
            tracker.reset();
        }
    }

    /// Clear all watched ranges and reset the access tracker
    pub fn clear_access_tracker(&mut self) {
        if let Some(tracker) = self.memory_mapping.get_access_tracker() {
            tracker.clear();
        }
    }
