...
use std::{array, cell::{Cell, RefCell, UnsafeCell}, fmt, mem, ops::Range, ptr};
...
/// Tracks memory accesses to detect reads from forbidden address ranges
/// Uses interior mutability to allow tracking from immutable contexts (e.g., map())
#[derive(Debug, Default)]
pub struct MemoryAccessTracker {
    /// List of forbidden address ranges to watch for reads
    watched_ranges: RefCell<Vec<Range<u64>>>,
    /// List of (address, size) tuples representing forbidden accesses detected
    forbidden_accesses: RefCell<Vec<(u64, usize)>>,
    /// Whether tracking is enabled
    enabled: Cell<bool>,
}

impl Clone for MemoryAccessTracker {
    fn clone(&self) -> Self {
        Self {
            watched_ranges: RefCell::new(self.watched_ranges.borrow().clone()),
            forbidden_accesses: RefCell::new(self.forbidden_accesses.borrow().clone()),
...
impl MemoryAccessTracker {
    pub fn new() -> Self {
        Self {
            watched_ranges: RefCell::new(Vec::new()),
            forbidden_accesses: RefCell::new(Vec::new()),
            enabled: Cell::new(false),
        }
    }

    /// Add a single forbidden range to watch
    pub fn add_watched_range(&self, range: Range<u64>) {
        if !range.is_empty() {
            self.watched_ranges.borrow_mut().push(range);
            self.enabled.set(true);
        }
    }

    /// Add multiple forbidden ranges to watch
    pub fn add_watched_ranges(&self, ranges: &[Range<u64>]) {
        let mut watched = self.watched_ranges.borrow_mut();
        for range in ranges {
            if !range.is_empty() {
                watched.push(range.clone());
            }
        }
        if !watched.is_empty() {
            self.enabled.set(true);
    ...
    /// Check if a memory read at vm_addr with given length overlaps with any watched range
    /// This can be called from immutable contexts thanks to interior mutability
    pub fn check_access(&self, vm_addr: u64, len: u64) {
        if !self.enabled.get() {
            return;
        }

        let read_end = vm_addr.saturating_add(len);
        let watched = self.watched_ranges.borrow();
        for watch_range in watched.iter() {
            // Check for overlap: read starts before watch ends AND read ends after watch starts
            if vm_addr < watch_range.end && read_end > watch_range.start {
                self.forbidden_accesses.borrow_mut().push((vm_addr, len as usize));
                // Only record once per access even if it overlaps multiple ranges
                break;
            }
        }
    }

    /// Returns true if any forbidden accesses were detected
    pub fn has_forbidden_accesses(&self) -> bool {
        !self.forbidden_accesses.borrow().is_empty()
    }

    /// Get a copy of the list of forbidden accesses detected
    pub fn get_forbidden_accesses(&self) -> Vec<(u64, usize)> {
        self.forbidden_accesses.borrow().clone()
    }

    /// Get a copy of the watched ranges
    pub fn get_watched_ranges(&self) -> Vec<Range<u64>> {
        self.watched_ranges.borrow().clone()
    }

    /// Reset the tracker, clearing all recorded accesses but keeping watched ranges
    pub fn reset(&self) {
        self.forbidden_accesses.borrow_mut().clear();
    }

    /// Clear all watched ranges and reset
    pub fn clear(&self) {
        self.watched_ranges.borrow_mut().clear();
        self.forbidden_accesses.borrow_mut().clear();
...
pub struct CommonMemoryMapping {
    ...
    /// Memory access tracker for detecting forbidden memory reads
    pub access_tracker: MemoryAccessTracker,
}
    ...
    pub fn new_with_access_violation_handler(
        ...
        let mut result = Self {
            common: CommonMemoryMapping {
                ...
                sbpf_version,
                access_tracker: MemoryAccessTracker::new(),
            },
...
impl MemoryMapping {
    ...
    pub fn map(&self, access_type: AccessType, vm_addr: u64, len: u64) -> ProgramResult {
        // Track memory access for Load operations (e.g., memcpy reads)
        if access_type == AccessType::Load {
            match self {
                MemoryMapping::Aligned(m) => m.common.access_tracker.check_access(vm_addr, len),
                MemoryMapping::Unaligned(m) => m.common.access_tracker.check_access(vm_addr, len),
                MemoryMapping::Identity => {},
            }
        }
    ...
    pub fn load<T: Pod + Into<u64>>(&mut self, vm_addr: u64) -> ProgramResult {
        ...
        // Track memory access before performing the read
        match self {
            MemoryMapping::Aligned(m) => m.common.access_tracker.check_access(vm_addr, len),
            MemoryMapping::Unaligned(m) => m.common.access_tracker.check_access(vm_addr, len),
            MemoryMapping::Identity => {}, // No tracking for identity mapping
        }
    ...
    /// Get reference to the access tracker
    pub fn get_access_tracker(&self) -> Option<&MemoryAccessTracker> {
        match self {
            MemoryMapping::Identity => None,
            MemoryMapping::Aligned(m) => Some(&m.common.access_tracker),
            MemoryMapping::Unaligned(m) => Some(&m.common.access_tracker),
        }
    }
}
